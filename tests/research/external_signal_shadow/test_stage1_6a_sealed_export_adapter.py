...
def test_snapshot_rejects_attestation_run_contract_manifest_or_header_profile_mismatch(tmp_path):
    ...
    rewrite_authoritative_artifact(export, "source_profile_probe_attestation.json", json.dumps(att).encode("utf-8"))
...
def test_network_error_then_trusted_observation_produces_trusted_parent(tmp_path):
    ...
    existing_obs = [json.loads(x) for x in obs_p.read_text(encoding="utf-8").splitlines() if x.strip()]
...
def test_two_trusted_observations_distinct_raw_hashes_select_max_trusted_time_then_hash(tmp_path):
    ...
    payload2 = json.dumps({
        ...
        "data": {
            ...
            "body": json.dumps({"node": "root", "child": [{"node": "element", "tag": "p", "child": [{"node": "text", "text": "Delist TOKENAUSDT"}]}]}),
    ...
    existing_rev = [json.loads(x) for x in rev_p.read_text(encoding="utf-8").splitlines() if x.strip()]
    ...
    rewrite_authoritative_artifact(export, "detail_observations/historical.jsonl", ("\n".join(json.dumps(x) for x in [existing_obs[0], obs2]) + "\n").encode("utf-8"))
...
def test_revision_profile_variant_header_or_surface_locale_mismatch_rejects_export(tmp_path):
    ...
    rewrite_authoritative_artifact(export, "detail_revisions.jsonl", (json.dumps(existing_rev[0]) + "\n").encode("utf-8"))
...
def test_present_wrong_data_code_rejects_entire_export(tmp_path):
    aid = "1" * 32
    wrong_aid = "9" * 32
    raw = json.dumps({"code": "000000", "data": {"id": 123, "code": wrong_aid, "title": "Delist", "body": "{}"}}).encode("utf-8")
    root, export = build_valid_historical_sealed_export(tmp_path, article_specs=[trusted_article(article_id=aid, raw_payload_bytes=raw)])
    snapshot = adapter.load_verified_source_snapshot(root, export)
    with pytest.raises(adapter.AdapterInputError):
        adapter.reduce_verified_snapshot(snapshot, semantic_extracted_at_ms=1700000050000, grammar_pair=adapter.G1_GRAMMAR_PAIR)
...
def test_body_unknown_tag_and_nonempty_br_child_are_body_parse_unresolved(tmp_path):
    aid = "1" * 32
    bad_body = json.dumps({"node": "root", "child": [{"node": "element", "tag": "script", "child": []}]})
...
def test_canonical_body_normalization_golden_vectors():
    ...
    body_tree = {
        "node": "root",
        "child": [
            {"node": "element", "tag": "p", "child": [{"node": "text", "text": "Paragraph 1\r\nwith CRLF."}]},
            {"node": "element", "tag": "br"},
            {"node": "element", "tag": "p", "child": [{"node": "text", "text": "  Paragraph   2 with spaces  "}]},
        ],
    }
...
def test_first_list_capture_missing_duplicate_wrong_article_or_invalid_release_date_rejects(tmp_path):
    aid = "1" * 32
    root, export = build_valid_historical_sealed_export(tmp_path, article_specs=[trusted_article(article_id=aid)])
    ...
    rewrite_authoritative_artifact(export, str(raw_index_p.relative_to(export)), json.dumps(raw_dict).encode("utf-8"))
...
def test_incompatible_trusted_revisions_produce_revision_conflicting_without_eligible_child(tmp_path):
    aid = "1" * 32
    ...
    payload2 = json.dumps({
        ...
        "data": {
            ...
            "body": json.dumps({"node": "root", "child": [{"node": "element", "tag": "p", "child": [{"node": "text", "text": "Delist USDⓈ-M TOKENAUSDT Perpetual at 2026-09-01 09:00 (UTC)."}]}]}),
            "publishDate": 1700000000000,
        },
    }).encode("utf-8")
    sha2 = hashlib.sha256(payload2).hexdigest()
...
def test_unresolved_batch_child_prevents_any_eligible_child_subset(tmp_path):
    ...
    body_tree = {
        "node": "root",
        "child": [
            {"node": "element", "tag": "p", "child": [{"node": "text", "text": "Binance Futures will delist USDⓈ-M TOKENAUSDT Perpetual at 2026-08-25 09:00 (UTC)."}]},
            {"node": "element", "tag": "p", "child": [{"node": "text", "text": "Binance Futures will also delist UNKNOWN_BATCH_TOKEN at 2026-08-25 09:00 (UTC)."}]},
        ],
    }
    raw = json.dumps({"code": "000000", "data": {"id": 123, "code": aid, "title": "Delist", "body": json.dumps(body_tree), "publishDate": 1700000000000}}).encode("utf-8")
    root, export = build_valid_historical_sealed_export(tmp_path, article_specs=[trusted_article(article_id=aid, raw_payload_bytes=raw)])
    ...
    assert not any(c.get("source_audit_eligible") for c in reduction.contracts if c.get("parent_article_id") == aid)
...
def test_all_frozen_candidates_require_valid_first_list_capture_chain(tmp_path):
    ...
    discoveries = [json.loads(x) for x in ad_p.read_text(encoding="utf-8").splitlines() if x.strip()]
    discoveries[0]["first_list_capture_id"] = "nonexistent_lc_id"
    rewrite_authoritative_artifact(export, "article_discoveries.jsonl", (json.dumps(discoveries[0]) + "\n").encode("utf-8"))
