...
8. Completion requires hard verification through tests, logs, or reproducible evidence. Where automated tests are applicable to the changed behavior, they are necessary but never sufficient evidence for completion of cross-boundary, persistence, provenance, restart, safety, or authority changes. Passing tests generated by the executor themselves must not be accepted without verifying production call paths, boundary contracts, and negative fail-closed behavior.
...
- Completion Audit returns `incomplete` or `blocked`: `.agent/workflows/remediate-completion-audit.md`
