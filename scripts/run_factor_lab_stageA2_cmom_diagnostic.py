...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Cross-Sectional Factor Lab Stage A2.2 CMOM diagnostic.")
    ...
    parser.add_argument("--fail-on-stop", action="store_true", help="Return non-zero exit code if next_action is stop_price_only_momentum or decision is data_unavailable")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.offline_sample:
        ...
        try:
            ...
            return 1 if args.fail_on_stop else 0

        daily_bars = fixture.get("daily_bars", [])
        summary = run_stageA2_cmom_diagnostic(daily_bars)

    else:
        logger.info("Running in live network mode fetching Binance spot daily bars...")
        if args.exchange.lower() != "binance":
            logger.error(f"Exchange {args.exchange} not supported for Stage A2.2 live run.")
            summary = {
                "stage": "stageA2_cmom_diagnostic",
    ...
    logger.info(f"Next Action: {summary.get('next_action')}")

    if args.fail_on_stop:
        if (
            summary.get("next_action") == "stop_price_only_momentum"
            or summary.get("decision") == "stageA2_cmom_data_unavailable"
        ):
            logger.error(f"Backtest halted. Decision: {summary.get('decision')}, Next Action: {summary.get('next_action')}")
            return 1
