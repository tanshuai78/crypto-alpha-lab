...
def build_route_b_5m_summary(
    ...
    *,
    ...
    return {
        ...
        "convert_to_usd": True,
        "vendor_granularity": "5min",
        "normalized_granularity": "5m",
    }


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Fetch 5m third party liquidation history from Coinalyze."
    )
    parser.add_argument(
        "--symbols",
        nargs="+",
    ...
    parser.add_argument("--interval", default="5min", help="Granularity interval, default 5min")
    ...
    parser.add_argument(
        "--summary-output",
        default="reports/liquidation_only_5m/liquidation_only_5m_fetch_summary.json",
    ...
    if not api_key:
        ...
        for idx, symbol in enumerate(args.symbols):
            ...
            logger.info(f"Fetching 5m liquidation history for {symbol}...")
            try:
                ...
                if payload:
                    normalized = normalize_coinalyze_payload_5m(payload, symbol=symbol)
    ...
    summary = build_route_b_5m_summary(
        all_normalized_rows,
    ...
    if args.output_jsonl:
        ...
        logger.info(f"Saved normalized 5m rows to {args.output_jsonl}")
